ScienScanJs = function() {
    ...
    var addTopicToSummary = function(topicId) {
        waitingOn();
        var query = $('#search-box').val();
        var url = '/labeling?query=' + query + '&topic-id=' + topicId;
    ...
    var banTopicFromSummary = function(topicId) {
        waitingOn();
        var query = $('#search-box').val();
        var url = '/ban?query=' + query + '&topic-id=' + topicId;
        $.post(url, function(data) {
            document.getElementById('topic-graph').innerHTML=data.svg;
            topicVisData = data.json;
            init();
    ...
    var isLabelingInterface = function() {
        return "labeling" == topicVisData["interface"];
    }
    ...
    var isLabelingComplete = function() {
        return topicVisData["complete-labeling"];
    }
    ...
    var init = function() {
        ...
        var nodeClickFn;
        if (isLabelingInterface()) {
            nodeClickFn = isLabelingComplete() ? ignore : function(){addTopicToSummary($(this).find('title').text());};
            topicNodes.bind('contextmenu',function(e){
                e.preventDefault();
                banTopicFromSummary($(this).find('title').text());
            });
        } else {
            nodeClickFn = function(){showTopic($(this).find('title').text());};
        }
        topicNodes.bind('click',nodeClickFn);
        if (!isLabelingInterface()) {
            showResults();
        }
    }
    ...
    var changeMinFreqValue = function(valueFn) {
        
        waitingOn();

        var sl = $('#minFreqSlider');
        var newValue = valueFn(sl.slider('value'));
        sl.slider('value', newValue);

        var query = $('#search-box').val();
        var url = '/refine?query=' + query + '&n-topics=' + newValue;
        $.post(url, function(data) {
            document.getElementById('topic-graph').innerHTML=data.svg;
            topicVisData = data.json;
            init();
...
$(document).ready(function () {
    ...
    $('#show-more').click(function() {
        ScienScanJs.refine(function(value) {return value + 1;});
    });
    $('#show-less').click(function() {
        ScienScanJs.refine(function(value) {return value - 1;});
    });
    $('#search-controls form').submit(function() {
        ScienScanJs.refine(function(value) {return value;});
        return false;
    });
