...
(def jaccard-based-loss
  (Loss. (fn jaccard-based-loss [optimal predicted]
           (let [topic-map (:topic-map optimal)
                 optimal-topics (:topics optimal)
                 predicted-topics (:topics predicted)
                 jaccard #(- 1 (/ (count (set/intersection %1 %2))
                                  (count (set/union %1 %2))))
                 predicted-topic-docs (map #(set (ftr/covered-docs topic-map [%])) predicted-topics)
                 topic-scores (for [topic optimal-topics
                                    :let [optimal-topic-docs (set (ftr/covered-docs topic-map [topic]))
                                          jaccard-scores (map (partial jaccard optimal-topic-docs) predicted-topic-docs)]]
                                (apply min jaccard-scores))]
             (* 1.0 (u/avg topic-scores))))))

(def distance-based-loss
  (Loss. (fn jaccard-based-loss [optimal predicted]
           (let [topic-map (:topic-map optimal)
                 topic-graph (:topic-graph topic-map)
                 optimal-topics (:topics optimal)
                 predicted-topics (:topics predicted)
                 optimal-topic (last optimal-topics)
                 predicted-topic (last predicted-topics)
                 dag-dist (g/dag-distance topic-graph optimal-topic predicted-topic)]
             (* 1.0 (if dag-dist (/ dag-dist (inc dag-dist)) 1))))))
...
(defn sum-loss [& loss-fns]
  (Loss. (fn [s1 s2]
           (apply + (map #(sl/compute-loss % s1 s2) loss-fns)))))
...
(def features
  (Features. (vec (concat
                   ...
                   ; #(ftr/v-structures %1 %2 :reversed false :count :nodes) ; 22 
                   ; #(ftr/v-structures %1 %2 :reversed true :count :nodes) ; 23 
                     ]
                  ; (ftr/vector-feature 4 ftr/distant-paper-coverage) ; 17-20
                  ; (ftr/vector-feature 4 #(ftr/distant-topic-coverage %1 %2 :count :topics)) ; 21-24
                  ; (ftr/vector-feature 4 #(ftr/distant-topic-coverage %1 %2 :count :freqs))
                    )))) ; 25-28

(defn read-input-output [query]
...
(defn state-action-seqs [queries]
  (->> queries (map read-input-output)))

;(def model (sl/train-model state-action-seqs loss01 features))
...
(defn accuracies [model query]
  (let [[init-state optimal-seq] (read-input-output query)
        n (count optimal-seq)
